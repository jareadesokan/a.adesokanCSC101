...
fn main() {
    ...
    println!("This will save the details of all the students you enter into this program!\n");
    let mut no_students = String::new();
    println!("\nHow many students do you want to ente details for? ");
    ...
    for i in 1..no_students + 1   {
        ...
        println!("Please input the Student's Matriculation Number.");
        ...
        println!("Please input the student's Department.");
        std::io::stdin().read_line(&mut input3).expect("Failed to read input");
        let new_dept = input3.trim().to_string();
        dept.push(new_dept);

        let mut input4 = String::new();
        println!("What Level is the student in? Please input in numbers.");
